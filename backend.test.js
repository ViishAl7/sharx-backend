...
async function run() {
  ...
  console.log("\n── Auth routes (Google / Microsoft OAuth — no email/password login exists) ──");

  await test("GET /auth/google redirects to Google's OAuth consent screen", async () => {
    const { status, headers } = await get("/auth/google");
    assert(status === 302 || status === 301, `expected a redirect (301/302), got ${status}`);
    const location = headers.get("location") || "";
    assert(location.includes("google.com") || location.includes("accounts.google"), `expected redirect to Google, got: ${location}`);
  });

  await test("GET /auth/microsoft redirects to Microsoft's OAuth consent screen", async () => {
    const { status, headers } = await get("/auth/microsoft");
    assert(status === 302 || status === 301, `expected a redirect (301/302), got ${status}`);
    const location = headers.get("location") || "";
    assert(
      location.includes("microsoft.com") || location.includes("login.live") || location.includes("microsoftonline"),
      `expected redirect to Microsoft, got: ${location}`
    );
  });

  await test("GET /auth/google/callback without OAuth code fails gracefully (not 500)", async () => {
    const { status } = await get("/auth/google/callback");
    assert(status !== 500, `server crashed with 500 on callback without a code`);
  });

  console.log("\n── User routes (real paths: /user/profile, /user/history — all protected) ──");
  ...
  await test("GET /user/profile with a garbage/invalid token returns 401", async () => {
    const { status } = await get("/user/profile", {
      headers: { Authorization: "Bearer this.is.not.a.valid.jwt" },
    });
    assert(status === 401 || status === 403, `expected 401/403 with invalid token, got ${status}`);
  });

  await test("GET /user/history without token returns 401", async () => {
    const { status } = await get("/user/history");
    assert(status === 401 || status === 403, `expected 401/403 without auth, got ${status}`);
  });

  await test("PUT /user/profile without token returns 401", async () => {
    const { status } = await put("/user/profile", { name: "Test Name" });
    assert(status === 401 || status === 403, `expected 401/403 without auth, got ${status}`);
  });
